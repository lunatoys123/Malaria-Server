...
export const Register = async (req, res) => {
  ...
  if (exist_hospital_id == null) {
    return res
      .status(400)
      .send({ status: status_code.Failed, Message: "Hospital id not exist" });
  }
  ...
  if (user) {
    return res
      .status(400)
      .send({ status: status_code.Failed, Message: "User exists already" });
  }
...
export const Login = async (req, res) => {
  ...
  if (!user) {
    return res
      .status(400)
      .send({ status: status_code.Failed, Message: "User Not exist" });
  }
  if (user && bcrypt.compareSync(Password, user.Password)) {
    ...
    return res
      .status(404)
      .send({ status: status_code.Failed, Message: "Wrong Password" });
  }
};
...
export const GetAllUser = async (req, res) => {
  ...
  return res
    .status(200)
    .send({ status: status_code.Success, user_list: All_User });
};
